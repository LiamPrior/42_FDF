...
/*   Updated: 2018/03/21 23:08:58 by lprior           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_directions(t_env *all)
{
    ...
    while (initial)
    {
        ...
        else if (initial->next && (initial->next->x > initial->next))//if its at the end
            ft_draw(initial, initial->next, all);
        else//the next link is less then the current one and down to next doesnt equal null keep going.
            ft_draw(initial, terminal, all);
        if (initial->next)
